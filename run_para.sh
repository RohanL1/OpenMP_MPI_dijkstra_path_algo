...
banner
for THD in 28 24 20 16 12 8 4 2
do
    #./dijkstra  ${GRPH} ${NSRCH} ${LOG}.data >> ${RUN_LOG} 2>&1
    echo "executing > ./dijkstra_omp  ${GRPH} ${NSRCH} ${THD} ${SOL_LOG}_para.data >> ${RUN_LOG} 2>&1"
    ./dijkstra_omp  ${GRPH} ${NSRCH} ${THD} ${SOL_LOG}_para.data >> ${RUN_LOG} 2>&1
done
